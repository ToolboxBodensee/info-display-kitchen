        ...
        for n, item in pairs(preise) do
            if item == Absatz then
                ...
                if sys.now() > next_kaputt - 3 and n == row then
                    ...
                    title = a .. c .. b .. d
                    font:write(x, yy, title, size, 1,1,1,1)
                ...
                font:write(x + spacing, yy, price, size, 1,1,1,1)
                yy = yy + size
            end
        end
    end
...
function Crapterm(source, x, y, size)
    ...
    function draw()
        ...
        current = "> " .. current
        local cursor_x = font:write(x, y, current, size, 1,1,1,1)
        cursor_x = cursor_x + 40
        if ((sys.now() - cursor_base)*2) % 2 < 1 then
            font:write(cursor_x, y, "_", size, 1,1,1,1)
        end
